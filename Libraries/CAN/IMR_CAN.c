...
void CAN_IRQ_RX_REDUCE_PWR_MSG_HANDLER(void) {	// Handler for Reduce Power message
	XMC_CAN_MO_Receive(&CAN_NODE_RX_REDUCE_PWR_LMO_NAME); // Receive data from CAN Node and transfer into CAN data structure

	uint32_t id = XMC_CAN_MO_GetIdentifier(&CAN_NODE_RX_REDUCE_PWR_LMO_NAME);
	uint8_t *data = CAN_NODE_RX_REDUCE_PWR_LMO_NAME.can_data_byte;

	if ((id == BMS_REDUCE_POWER_FOR_HS) && (!BMS_PowerReduced)) { // Set motor speed targets to zero and disable further motor messages
		NVIC_DisableIRQ(TIMER_TIMEOUT_PERIOD_MATCH_EVENT_IRQN);		// Enable CCU4 timer period match interrupt
		XMC_GPIO_SetOutputHigh(LED_Blue_PORT, LED_Blue_PIN);
		BMS_PowerReduced = true;	// Set flag for main-loop to reduce IMR Power (Motors, Optional: Sensor LEDs)
...
void CAN_IRQ_RX_ENABLE_PWR_MSG_HANDLER(void) { // Handler for Enable Power message
	XMC_CAN_MO_Receive(&CAN_NODE_RX_ENABLE_PWR_LMO_NAME); // Receive data from CAN Node and transfer into CAN data structure

	uint32_t id = XMC_CAN_MO_GetIdentifier(&CAN_NODE_RX_ENABLE_PWR_LMO_NAME);
	uint8_t *data = CAN_NODE_RX_ENABLE_PWR_LMO_NAME.can_data_byte;

	if ((id == BMS_ENABLE_POWER_FOR_HS) && (BMS_PowerReduced)) { // Enable further motor messages
		NVIC_EnableIRQ(TIMER_TIMEOUT_PERIOD_MATCH_EVENT_IRQN); // Enable CCU4 timer period match interrupt
		BMS_PowerReduced = false;	// Reset flag for main-loop to resume normal power operation
	}
}

void CAN_IRQ_RX_VELOCITY_MSG_HANDLER(void) { // Handler for Trajectory Command message
	XMC_CAN_MO_Receive(&CAN_NODE_RX_VELOCITY_LMO_NAME); // Receive data from CAN Node and transfer into CAN data structure
	...
	if (((id == ROBOT_VELOCITY_COMMAND) && (!BMS_PowerReduced))) {
		Trajectory_ttl = TRAJECTORY_FREQUENCY;	// By setting the Trajectory_ttl to a value > 0. The Trajectory commands will be processed in main.c
...
void CAN_IRQ_RX_ENCODER_FL_MSG_HANDLER(void) { // Handler for Encoder message
	...
	if (((id == MOT_FL_ENCODER_DATA) && (!BMS_PowerReduced))) {
		...
		encoderSpeed[0] = ((float)((int16_t)((data[0] << 8) + data[1]))) / RADPS2_15BIT;	// Getting [rad/s] from CAN [pu]
	}
}
...
void CAN_IRQ_RX_ENCODER_FR_MSG_HANDLER(void) {
	...
	if (((id == MOT_FR_ENCODER_DATA) && (!BMS_PowerReduced))) {
		...
		encoderSpeed[1] = ((float)((int16_t)((data[0] << 8) + data[1]))) / RADPS2_15BIT;	// Getting [rad/s] from CAN [pu]
	}
}
...
void CAN_IRQ_RX_ENCODER_BL_MSG_HANDLER(void) {
	...
	if (((id == MOT_BL_ENCODER_DATA) && (!BMS_PowerReduced))) {
		...
		encoderSpeed[2] = ((float)((int16_t)((data[0] << 8) + data[1]))) / RADPS2_15BIT;	// Getting [rad/s] from CAN [pu]
	}
}
...
void CAN_IRQ_RX_ENCODER_BR_MSG_HANDLER(void) {
	...
	if (((id == MOT_BR_ENCODER_DATA) && (!BMS_PowerReduced))) {
		...
		encoderSpeed[3] = ((float)((int16_t)((data[0] << 8) + data[1]))) / RADPS2_15BIT;	// Getting [rad/s] from CAN [pu]
	}

	//Bargraph Pattern similar to embedded world demo
	if (id == BAR_GRAPH) {
...
void CAN_Initialize(void) {
	...
	NVIC_EnableIRQ(CAN_IRQ_RX_REDUCE_PWR_NUMBER);	// Enable NVIC IRQ with correct IRQ number: see Reference Manual
